...
public class CloudFoundryJavaClientConfig {
	...
	@Bean
	@ConditionalOnProperty({PROPERTY_PREFIX + ".username",
			PROPERTY_PREFIX + ".password"})
	PasswordGrantTokenProvider passwordGrantTokenProvider(CloudFoundryProperties properties) {
	...
	@Bean
	@ConditionalOnProperty({PROPERTY_PREFIX + ".client-id",
			PROPERTY_PREFIX + ".client-secret"})
